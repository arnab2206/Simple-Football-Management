...
<hr/>
Current remaining matches:<br/>
<?php
$played=0;
$sql="SELECT match_id FROM matches where played='$played'";
$result=mysqli_query($conn,$sql);
if($result)
{
    $row=mysqli_num_rows($result);
...
?>
<hr/>
Current No. of players in the league:<br/>
<?php
...
<hr/>
Register new player under an existing team:<br/>
...
<input type="submit" name="newPlayer" value="Register new player"/>
<hr/>
...
<?php
    ...
    if(!empty($_REQUEST['newPlayer']))
    {
        $name=$_REQUEST['playerName'];
        $team=$_REQUEST['team'];
        $pos=$_REQUEST['pos'];
        $sql="INSERT INTO players values('','$name','$team','$pos')";
        if(mysqli_query($conn,$sql))
        {
            echo "Player $name added successfully to team $team!";
        }
        else
        {
            echo mysqli_error($conn);
